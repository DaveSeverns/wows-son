...
enum class NavScreen(val icon: ImageVector,  @StringRes val label: Int? = null, val navRoute: String) {
    WowMovies(VectorIcons.WowIcon, label = R.string.movies_label, navRoute = ScreenKeys.WOW_MOVIES_KEY),
    Favorites(Icons.Filled.Favorite,label = R.string.favorites_label, navRoute = ScreenKeys.WOW_FAVORITES_KEY)
}